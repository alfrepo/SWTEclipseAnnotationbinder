...
class EvaluatorEnability {
     ...
     */
    void bindWidgetsEnabilityAttribute(Field annotatedField, Object annotatedFieldValue,
            BindWidgetEnabilityToModel bindWidgetEnabilityToModel) throws NoSuchFieldException, SecurityException {

        // skip if there is no such annotation
        if (bindWidgetEnabilityToModel == null) {
            return;
        }

        // retrieve ClassOfModel
        Class<?> modelClass = bindWidgetEnabilityToModel.modelClass();
        // retrieve FieldInModel
        String modelFieldName = bindWidgetEnabilityToModel.modelFieldName();
        // retrieve WidgetRetriever
        Class<? extends IWidgetExtractor> widgetRetrieverClass = bindWidgetEnabilityToModel.widgetExtractor();
        ...
        bindWidgetpropertyEnabilityToModel(bindWidgetEnabilityToModel, modelFieldName, model, widget, annotatedFieldValue);
    }

    /** do the binding between field and widget' text property */
    private void bindWidgetpropertyEnabilityToModel(BindWidgetEnabilityToModel bindingToSelectionAnnotation,
        ...
        try {
            observeSelectionInWidget = WidgetProperties.enabled().observe(widget);
        } catch (Exception e) {
        ...
        /*
         * For the enability no Validators and no converters needed.
         * The validators should not be started with a boolean value, every time the enability changes.
         * So an empty container with BindingContextParams is passed.
         * Otherwise the BindingContextParams from the context would be used.
         */
        BindingContextParams empty = new BindingContextParams();

        // do the binding
        evaluatorTools.bindWithUpdateStrategy(jfaceDataBindingContext, annotatedFieldValue, observeSelectionInWidget, observeModelfieldInPojo, empty);
    }
